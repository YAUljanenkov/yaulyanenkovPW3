...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let images = [
            UIImage(named: "stack")?.resize(withSize: CGSize(width: 32, height: 32)),
            UIImage(named: "table")?.resize(withSize: CGSize(width: 32, height: 32)),
            UIImage(named: "collection")?.resize(withSize: CGSize(width: 32, height: 32))
        ]
